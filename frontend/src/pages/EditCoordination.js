...
const EditCoordination = () => {
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            const coordinatesArray = activeUserObject.coordinates.filter( (course) => (course.courseNumber))
        ...
        <div className="mt-4 mx-2 col-md-4">
            ...
            <form onSubmit={e => submit(e)}>
                <div className="form-group">
                    ...
                    { activeUserObject && activeUserObject.coordinates[0] && activeUserObject.coordinates.map((c, i) => (
                        <div className="row" key={i}> 
                            <div className="col-md-6"><label className="form-label mb-0 mt-1">Course Number</label><input className="form-control" value={c.courseNumber} 
                                onChange={(e)=>{ 
                                    let temp = activeUserObject.coordinates
                                    temp[i].courseNumber = e.target.value
                                    setActiveUserObject({ coordinates: temp })}}/>
                            </div> 
                        </div>
                    )) }
                </div>
